        ...
        true ->
            ...
            case Filtered of
                [] ->
                    {ok, []};
                _ ->
                    antidote_pvc_protocol:read_keys(Filtered, TxId)
            end
    end.
...
safe_index_read(Keys, TxId) ->
    case antidote_pvc_protocol:read_keys(Keys, TxId) of
        {error, Reason} ->
            {error, Reason};
        {ok, ReadValues} ->
...
claimed_index(RootKey, TxId) ->
    {ok, [RootVal]} = antidote_pvc_protocol:read_keys(RootKey, TxId),
    RootVal =:= ?CLAIMED.
...
update_indices(Updates, TxId = #tx_id{server_pid = Pid}) ->
    ok = antidote_pvc_protocol:update_keys(Updates, TxId),
