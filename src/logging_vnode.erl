...
init([Partition]) ->
    ...
    case open_logs(LogFile, Preflists, OpIdTable) of
        {error, Reason} ->
...
%%                     to the logs.
%%
%%          In addition ClockTable will be updated with
%%          entries from the opened log.
%%
-spec open_logs(string(), [preflist()], cache_id()) -> {dict:dict(), vectorclock()} | {error, reason()}.
open_logs(LogFile, PrefLists, OpIdTable) ->
    open_logs(LogFile, PrefLists, dict:new(), OpIdTable, vectorclock:new()).
...
open_logs(LogFile, [Next|Rest], Map, ClockTable, MaxVector)->
    ...
    case disk_log:open([{name, LogPath}]) of
        {ok, Log} ->
            {ok, NewMaxVector} = get_last_op_from_log(Log, start, ClockTable, MaxVector),
