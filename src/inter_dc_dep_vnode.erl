        ...
        {value, Txn} ->
            ...
            case Success of
                ...
                true ->
                    ...
                    process_queue(DCID, {pop_txn(NewState, DCID), Acc + 1})
            end
    end.
...
try_store(State, Txn = #interdc_txn{
    dcid = DCID,
    partition = Partition,
    timestamp = Timestamp,
    log_records = Ops
...
get_partition_clock(State) ->
    ...
    vectorclock:set_clock_of_dc(
        dc_meta_data_utilities:get_my_dc_id(),
        dc_utilities:now_microsec(),
        State#state.vectorclock
    ).
...
updates_to_clocksi_payloads(Txn = #interdc_txn{
    dcid = DCID,
